...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Symphony.Ltd.Web` and `Symphony.Ltd.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p f7891a07-8fd5-4686-8dcc-b4e8b2585e53
```

> `f7891a07-8fd5-4686-8dcc-b4e8b2585e53` is the password of the certificate, you can change it to any password you want.
...
Run `Symphony.Ltd.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `Symphony.Ltd.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `Symphony.Ltd.Web`: ASP.NET Core MVC / Razor Pages application that is the essential web application of the solution.
